<script lang="ts">
	...
	import { asError, isErrorLike, type ErrorLike } from "@sourcegraph/common/src/errors";

	import { checkMirrorRepositoryConnection } from "@sourcegraph/web/src/site-admin/backend";
    ...
    function check(repoName: string): Promise<ErrorLike|boolean> {
        return checkMirrorRepositoryConnection({ name: repoName }).pipe(
            ...
            {:then canAddOrError}
                {#if canAddOrError === true}
                    <p>
                        ...
                        users to search and view it by <a href="/site-admin/external-services">connecting an external service</a>
                    </p>
